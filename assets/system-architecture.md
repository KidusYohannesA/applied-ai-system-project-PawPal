...
| 🟡 **Amber** | Observability | Logger, test suite |

## Where humans + automated checks verify AI output

The system has **four checkpoints** between an AI response and a saved task:
...
4. **Test suite** ([tests/test_rag.py](../tests/test_rag.py)) — 7 mocked-Anthropic tests assert that bad schemas (invalid priority, out-of-range duration, missing citations, API errors) all return `None` without crashing. 1 real-API integration test verifies sensible suggestions for a known pet/task pair.
